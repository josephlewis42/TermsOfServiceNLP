...
def extract_examples(classname):
    ...
    for k in tos.classes.keys():
    	report_features(k, text_clf6)
    	
def report_features(classname, pocessor):
    examples = []
    ...
    for jdoc in find_json():
        ...
        with open(jdoc) as fd:
            ...
            for paragraph in marked["paragraphs"]:
                ...
                result = int(classname in classes)
                doc.append((jdoc, text, result))
...
if __name__ == "__main__":
    ...
    #text_clf6_classifier = OneVsRestClassifier(LinearSVC(class_weight={1:2,2:2,3:2,4:2,0:2,5:2,6:2,7:2,8:2,9:1}))
