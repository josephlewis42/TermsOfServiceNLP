...
@lru.lru_cache(50000)
def word_tokenize_lru(x, remove_stopwords=False):
	tokens = nltk.tokenize.word_tokenize(x)
	if remove_stopwords:
		tokens = filter(lambda x: not x.lower() in stopwords, tokens)
	return tokens
...
class EntityTransform(sklearn.base.TransformerMixin):
	...
	def transform(self, X, **transform_params):
		...
		for text in X:
			...
			for w in words:
				if w in set(['government', 'enforcement']):
					b.append(1)
					break
			else:
				b.append(0)
			for w in words:
				if w in set(['party','third-party']):
					b.append(1)
					break
			else:
				b.append(0)
...
class BOWTransform(sklearn.base.TransformerMixin):
	...
	def process(self, X):
		return np.array([word_tokenize_lru(x, True) for x in X])
