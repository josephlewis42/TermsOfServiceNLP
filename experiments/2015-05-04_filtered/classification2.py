...
def sklearn_multiclass_document(classnames, preprocessor, text_clf, reportwrong=False, ignorenone=False, filter_none=True):
    ...
    if filter_none:
        nonelabel = len(labels) - 1
        y2 = map(lambda x: x[nonelabel] == 1, y)
        y2 = map(int, y2)
        noneclassifier = SVC(kernel="linear", class_weight={0:2, 1:.5}, C=4.0)
        noneclassifier = noneclassifier.fit(x, y2)

        print "Filter classifier results:"

        a, b = zip(*test_set)
        b = mlb.transform(b)
        b = map(lambda x: x[nonelabel] == 1, b)
        b = map(int, b)
        predicted = noneclassifier.predict(a)
        print "Correct mean: {}".format(numpy.mean(predicted == b))
        print(metrics.classification_report(b, predicted))

        print len(x)
        predicted_none = noneclassifier.predict(x)
        print len(y)
        print len(predicted_none)
        x = numpy.matrix([r for i, r in enumerate(x) if predicted_none[i] == 0])
        y = numpy.matrix([r for i, r in enumerate(y) if predicted_none[i] == 0])
    ...
    if filter_none:
        ...
        nones_y =       [t for i, t in enumerate(y) if predicted_none[i] == 1]
        nones_x =       [t for i, t in enumerate(x) if predicted_none[i] == 1]
        notnones_y =    [t for i, t in enumerate(y) if predicted_none[i] == 0]
        notnones_x =    [t for i, t in enumerate(x) if predicted_none[i] == 0]

        print len(nones_y)
        print len(nones_x)
        print len(notnones_y)
        print len(notnones_x)

        onlynone = [0] * len(labels)
        onlynone[-1] = 1
        nones_predicted_classes = [onlynone] * len(nones_y)
        print len(nones_predicted_classes)

        other_predicted = text_clf.predict(numpy.matrix(notnones_x))

        nones_y.extend(notnones_y)
        y = numpy.matrix(nones_y)
        print y

        nones_predicted_classes.extend(other_predicted)
        predicted = numpy.matrix(nones_predicted_classes)
        print predicted
    else:
        predicted = text_clf.predict(x)

        x = [r for i, r in enumerate(x) if predicted_none[i] == 0]
        y = [r for i, r in enumerate(y) if predicted_none[i] == 0]
...
if __name__ == "__main__":
    ...
    sklearn_multiclass_document(list(tos.classes.keys()), preprocessor=text_clf6, text_clf=text_clf6_classifier, reportwrong=True, filter_none=True)
    exit()
